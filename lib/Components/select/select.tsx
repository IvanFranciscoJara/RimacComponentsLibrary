...
import {Options, ContainerOptions, ContainerInput, SelectContainer, OptionSelected} from './styled';
...
export const Select: React.FC<Props> = props => {
    ...
    const [ optionSelected, setOptionSelected ] = useState<{ value: string, text: string} | null>(null);
    const { placeholder, width, options, onSelect, selectedItem, disabled } = props;
                    ...
                    { options && options.map( option =>
                        <Options
                            ...
                            { optionSelected && (optionSelected.value === option.value &&
                                <Icon name={"gl_check"} width="16" height="16px" fill={COLORS.GREEN} />)
                            }
                        </Options>
                    )}
                </ContainerOptions>
