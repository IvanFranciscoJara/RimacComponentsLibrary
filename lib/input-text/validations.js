export default class Val {
    ...
    static documento(value, tipo){
        return  tipo === "1" ? Val.soloNumero(value) && value.length === 11 && ( value.substring(0,2) === "10" || value.substring(0,2) === "20" ) :
            tipo === "2" ? Val.soloNumero(value) && value.length === 8 :
                tipo === "3" || tipo === "4" ? value.length > 7 && value.length < 13 : false;
    }
    ...
    static placa(value){
        ...
        if(regex.test(value) && value.length == 6){
            return Val.soloTexto(value.substring(0,1)) && Val.soloNumero(value.substring(3,6));
        }else{
            return false;
        }
    }
    ...
    static anio(value){
        return Val.soloNumero(value) && 1960 <= value && value <= (new Date().getFullYear());
    }
